...
- 실제 환경의 문제를 정의 후 컴퓨터 언어로 바꾸고 동작가능한 프로그램으로 구현하는 것까지를 Software Development라고 말한다
  - 실제 환경의 `문제 정의(Oral로 설명)` 후 => `1단계`,
  - 이를 Programming Language로 `Solution`을 작성하며 객체지향/절차지향 2가지 방법이 있음 => `2단계`,
...
### **정리**
- `OOA` : Domain Model을 만들어서 Domain 레벨에서 Object/Class/Concepts를 찾고, UseCase Model을 만들어서 Requirement를 정리하는 것이다 -> `유즈케이스들을 모면서 클래스를 발굴!!!!`
...
## Rational Unnified Programming(RUP/UP)
...
> Iterative 방식에 Standard가 UP이며 notaion은 UML을 사용한다